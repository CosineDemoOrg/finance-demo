...
class TestFeeChangeBanner(unittest.TestCase):
    ...
    def test_fee_change_banner_contains_correct_message(self, mock_api_get):
        """Banner contains the correct fee change message mentioning 1.5%."""
        mock_api_get.side_effect = _mock_api_get_side_effect(balance=50000)
    ...
    def test_fee_rate_constant_is_one_point_five_percent(self):
        """TRANSACTION_FEE_RATE constant is 0.015 (1.5%)."""
        from decimal import Decimal
        self.assertEqual(self.app_module.TRANSACTION_FEE_RATE, Decimal('0.015'))
